...
global $xls_row;
	...
	else $header='<td class="head"><b>'.$title.'</b></td>';
	return $header;
}
...
$havebreak=false;
	...
	foreach ($rowheaders as $col=>$val) {
		...
		if(isset($columns[$col])) { // have a definition
			...
			if($columns[$col]['column_hide']) continue;
			else {
...
//	$report_parameter_form=getParameterForm($report_id,$parameters,false,true);
	...
	$op='run';
	if(count($parameters)==0) { $body=''; $xls_row=1; }
	else { $body=$report_parameter_form; $xls_row=2+count($parameters); }
		...
		if ($result) {
			if($myrow=$xoopsDB->fetchArray($result)) {
				...
				if(!$section_skipempty) {
					$body.='<table><tr>';
					++$xls_row;
					if($section_showtitle) { $body.="<th>$section_name</th></tr><tr>"; ++$xls_row; }
